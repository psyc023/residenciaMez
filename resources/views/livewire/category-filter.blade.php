    ...
    <div class="bg-white rounded-lg shadow-lg mb-6">
        <div class="px-6 py-2 flex justify-between items-center">
            ...
            <div class="hidden md:block grid-cols-2 border border-gray-200 divide-x divide-gray-200 text-gray-500">
                <i class="fas fa-border-all p-3 cursor-pointer {{ $view == 'grid' ? 'text-green-900' : ''}}" wire:click="$set('view', 'grid')"></i>
                <i class="fas fa-th-list p-3 cursor-pointer {{ $view == 'list' ? 'text-green-900' : ''}}" wire:click="$set('view', 'list')"></i>
            </div>
        </div>
    </div>
        ...
        <aside>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->subcategories as $subcategory)
                    <li class="py-2 text-sm">
                        <a class="cursor-pointer hover:text-green-900 capitalize {{ $subcategoria == $subcategory->slug ? 'text-green-900 font-semibold' : '' }}"
            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a class="cursor-pointer hover:text-green-900 capitalize {{ $marca == $brand->name ? 'text-green-900 font-semibold' : ''}}"
